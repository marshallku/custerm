...
//! `$XDG_CONFIG_HOME/turm/slack-tokens-<workspace>.json` (mode 0600,
//! atomic-replace via per-call temp + rename) with a stderr warning
//! on every open. `TURM_SLACK_REQUIRE_SECURE_STORE=1` forbids the
...
const KEYRING_SERVICE: &str = "turm-slack";
    ...
    match KeyringStore::open(&config.workspace_label) {
        ...
        Err(e) => {
            if config.require_secure_store {
                eprintln!(
                    "[slack] secure keyring unavailable AND TURM_SLACK_REQUIRE_SECURE_STORE=1: {e}"
                );
