...
//! - `TURM_SLACK_BOT_TOKEN` — `xoxb-...` Bot User OAuth Token. Used for
//!   HTTP API calls (`auth.test` to validate, future `chat.postMessage`).
//! - `TURM_SLACK_APP_TOKEN` — `xapp-...` App-Level Token with
//!   `connections:write` scope. Used by Socket Mode to open a
//!   WebSocket without needing a public HTTPS endpoint.
//!
//! `TURM_SLACK_WORKSPACE` is an optional namespacing label so a future
//! multi-workspace turm can run multiple slack plugin instances. v1 reads
...
impl Config {
    ...
    /// tokens are missing, so a one-time `turm-plugin-slack auth` is
    ...
    pub fn from_env() -> Result<Self, String> {
        let bot_token = std::env::var("TURM_SLACK_BOT_TOKEN").unwrap_or_default();
        if !bot_token.is_empty() && !bot_token.starts_with("xoxb-") {
            return Err(
                "TURM_SLACK_BOT_TOKEN, when set, must be a Bot User OAuth Token (xoxb-...)"
                    .to_string(),
            );
        }
        let app_token = std::env::var("TURM_SLACK_APP_TOKEN").unwrap_or_default();
        if !app_token.is_empty() && !app_token.starts_with("xapp-") {
            return Err(
                "TURM_SLACK_APP_TOKEN, when set, must be an App-Level Token (xapp-...) with connections:write"
                    .to_string(),
            );
        }
...
pub fn validate_workspace_label(s: &str) -> Result<(), String> {
    if s.is_empty() {
        return Err("TURM_SLACK_WORKSPACE: cannot be empty".to_string());
    }
    if s == "." || s == ".." {
        return Err(format!(
            "TURM_SLACK_WORKSPACE: {s:?} is reserved (use a normal label)"
        ));
    }
    for c in s.chars() {
        ...
        if !ok {
            return Err(format!(
                "TURM_SLACK_WORKSPACE: invalid character {c:?} \
