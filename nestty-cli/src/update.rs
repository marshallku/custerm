...
const REPO: &str = "marshallku/nestty";
...
fn client() -> reqwest::blocking::Client {
    reqwest::blocking::Client::builder()
        .user_agent(format!("nestty/{CURRENT_VERSION}"))
        .build()
...
pub fn check_update() {
    ...
    if latest > current {
        ...
        println!("Run `nestctl update apply` to install.");
    } else {
...
pub fn apply_update(version: Option<String>) {
    ...
    let asset_name = format!("nestty-{}-x86_64-linux.tar.gz", release.tag_name);
    let asset = release
        .assets
        .iter()
    ...
    println!("Downloading nestty v{target_version}...");
    ...
    println!("nestty v{target_version} installed successfully!");
}
