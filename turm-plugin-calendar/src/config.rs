...
//! because turm is OSS and embedding shared credentials would let anyone
//! impersonate "turm" in OAuth consent screens. Users supply their own
...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        let client_id = std::env::var("TURM_CALENDAR_CLIENT_ID")
            .map_err(|_| "TURM_CALENDAR_CLIENT_ID is required".to_string())?;
        ...
        let lead_minutes = parse_lead_minutes(
            &std::env::var("TURM_CALENDAR_LEAD_MINUTES").unwrap_or_else(|_| "10".to_string()),
        )?;

        let poll_secs: u64 = parse_nonzero_int("TURM_CALENDAR_POLL_SECS", 60)?;
        let lookahead_hours: u32 = parse_nonzero_int("TURM_CALENDAR_LOOKAHEAD_HOURS", 24)?;
        let require_secure_store = parse_bool("TURM_CALENDAR_REQUIRE_SECURE_STORE", false)?;
...
fn validate_account_label(s: &str) -> Result<(), String> {
    if s.is_empty() {
        return Err("TURM_CALENDAR_ACCOUNT: cannot be empty".to_string());
    }
    if s == "." || s == ".." {
        return Err(format!(
            "TURM_CALENDAR_ACCOUNT: {s:?} is reserved (use a normal label)"
        ));
    }
    for c in s.chars() {
        ...
        if !ok {
            return Err(format!(
                "TURM_CALENDAR_ACCOUNT: invalid character {c:?} \
...
fn parse_lead_minutes(raw: &str) -> Result<Vec<u32>, String> {
    ...
    if out.is_empty() {
        return Err("TURM_CALENDAR_LEAD_MINUTES: at least one value required".to_string());
    }
    out.sort_unstable();
    out.dedup();
    Ok(out)
