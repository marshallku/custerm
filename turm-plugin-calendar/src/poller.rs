...
//!    don't leak before turm has finished the handshake).
...
//!    run `turm-plugin-calendar auth` while turm is already running).
...
//!    band?" check is what gives us startup-catchup: if turm restarts
...
impl Poller {
    ...
    pub fn run(&self) {
        ...
        // the time between turm startup and the first scheduled tick
        // is permanently missed. Example: turm starts at 09:50:30,
...
mod tests {
    ...
    fn fires_after_restart_inside_firing_window() {
        // Reproduces codex round-5 C1: turm restarts at 09:50:30, an
