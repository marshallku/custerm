...
//! - `PlaintextStore` — JSON at `$XDG_CONFIG_HOME/turm/calendar-token-<account>.json`,
...
const KEYRING_SERVICE: &str = "turm-calendar";
    ...
    match KeyringStore::open(&config.account_label) {
        ...
        Err(e) => {
            if config.require_secure_store {
                ...
                eprintln!(
                    "[calendar] secure keyring unavailable AND TURM_CALENDAR_REQUIRE_SECURE_STORE=1: {e}"
                );
