...
//! revoked — caller must re-run `turm-plugin-calendar auth`).
...
impl Client {
    pub fn new(config: Config, store: Arc<dyn TokenStore>) -> Result<Self, String> {
        let tokens = store
            .load()
            .ok_or_else(|| "no stored credentials — run `turm-plugin-calendar auth`".to_string())?;
        Ok(Self {
            config,
            store,
            tokens,
            ...
            if page + 1 == MAX_PAGES {
                eprintln!(
                    "[calendar] list_events truncated after {MAX_PAGES} pages; \
                     consider narrowing TURM_CALENDAR_LOOKAHEAD_HOURS"
                );
            }
        }
        Ok(all)
