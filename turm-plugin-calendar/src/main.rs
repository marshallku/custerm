//! First-party Google Calendar service plugin for turm.
//!
...
//! - **(no args)** — RPC mode. Speaks the turm service-plugin protocol
...
//!   events at the lead times configured via `TURM_CALENDAR_LEAD_MINUTES`.
//!
...
//! This lets the user run `turm-plugin-calendar auth` while turm is
...
//! ship. Linux + macOS is turm's full support matrix; if/when a
//! Windows port lands, add `windows-native` to the keyring features
//! and relax this gate.

#[cfg(not(unix))]
compile_error!(
    "turm-plugin-calendar is currently Unix-only. The keyring crate's mock fallback \
...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        Some(other) => {
            ...
            eprintln!("usage: turm-plugin-calendar [auth]");
            std::process::exit(2);
        }
        None => run_rpc(),
    }
...
fn run_auth() {
    ...
    match oauth::run_device_code_flow(&config) {
        Ok(tokens) => {
            ...
            eprintln!("[calendar] auth ok — tokens stored, you can now start turm");
        }
        Err(e) => {
...
fn run_rpc() {
    let config = match Config::from_env() {
        ...
        Err(e) => {
            ...
            // turm is running. Log loudly so the cause is obvious.
...
fn handle_action(
    ...
    if config.is_minimal() {
        return Err((
            ...
            "TURM_CALENDAR_CLIENT_ID / TURM_CALENDAR_CLIENT_SECRET not set".to_string(),
        ));
    }
    match name {
