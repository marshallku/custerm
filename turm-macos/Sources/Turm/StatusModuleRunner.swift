...
///
/// Output protocol (matches `turm-linux/src/statusbar.rs::parse_output`):
...
final class StatusModuleRunner: @unchecked Sendable {
    ...
    init(
        ...
        queue = DispatchQueue(label: "turm.statusbar.\(moduleName)", qos: .utility)
    }
    ...
    private func runOnce() {
        ...
        env["TURM_PLUGIN_DIR"] = pluginDir.path
        ...
        do {
            ...
            FileHandle.standardError.write(Data("[turm] statusbar \(moduleName) spawn failed: \(error)\n".utf8))
            return
        }
        ...
        if process.terminationStatus != 0 {
            ...
            FileHandle.standardError.write(Data("[turm] statusbar \(moduleName) exec error: \(errStr)\n".utf8))
            return
        }
    ...
    /// matches `turm-linux/src/statusbar.rs::parse_output` byte-for-byte:
