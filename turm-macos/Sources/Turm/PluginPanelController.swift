...
/// Tier 4.1 — host-rendered plugin panel. Mirrors `turm-linux/src/plugin_panel.rs`:
/// loads the plugin's `panel.html` in a WKWebView, injects a `window.turm` JS
/// API at document-start so panel code can call back into turm via a Promise-based
...
/// macOS 14). Each `window.turm.call(...)` becomes a `postMessage(JSON)` →
...
/// surface that `turmctl call` reaches are both available from panel JS.
///
/// Event delivery: subscribe to `EventBus`, and on each event push
/// `turm._handleEvent(type, data)` into the webview via `evaluateJavaScript`.
...
@MainActor
final class PluginPanelController: NSViewController, TurmPanel {
    ...
    override func loadView() {
        ...
        // window.turm immediately on first run. Identical contract to Linux's
    ...
    /// Linux's `EventBus.subscribe("*")` + JS `turm._handleEvent` push.
    private func startEventForwarding() {
        ...
        Thread.detachNewThread { [weak self] in
            while let json = channel.receive() {
                ...
                let js = "if (window.turm && window.turm._handleEvent) turm._handleEvent(\(typeJSON), \(dataJSON))"
...
/// Bridges `window.webkit.messageHandlers.turm.postMessage(json)` from JS into
