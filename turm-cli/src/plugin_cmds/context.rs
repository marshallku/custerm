//! `turmctl context` aggregator (Phase 19.2 slice a).
//!
//! Default `turmctl context` (no flags) and `turmctl context --full`
...
pub fn dispatch(socket_path: &str, json_out: bool) -> i32 {
    ...
    // session.info contract is platform-divergent: turm-linux respects
    // the `id` param and returns that panel's info, but turm-macos's
    // current impl ignores `id` and returns active-tab info regardless
    // (turm-macos is stub-only per project memory; tracked as a
    ...
    // `worktree_root` (turm-plugin-git/src/main.rs::action_status).
