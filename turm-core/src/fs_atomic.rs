...
//!
//! Used by KB-shape plugins (`turm-plugin-kb`, `turm-plugin-todo`,
//! `turm-plugin-bookmark`) that store one logical record per file and need
...
compile_error!(
    "turm_core::fs_atomic only supports Linux (renameat2) and macOS (renamex_np). \
