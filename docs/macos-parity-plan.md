...
6. **PR 5b: event-producing plugin OR trigger engine** — Slack/Calendar/Discord (now that Keychain is unblocked) for the event side, then TriggerEngine FFI to wire `event.publish` from plugin → EventBus → trigger match → action dispatch. Slack is the highest user-value option (mention/DM/reaction events drive useful notifications) but has the heaviest setup (App-Level Token + Bot Token); Calendar is the simplest (poll mode, no websocket) and gives lead-time event proof for the trigger engine.
