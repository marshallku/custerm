...
2. **PR 2: macOS registry-first dispatch seam** — replace `AppDelegate.handleCommand` mega-switch with an `ActionRegistry`-equivalent that mirrors Linux's `actions.try_dispatch` pattern (`turm-linux/src/socket.rs:223`); legacy handlers fall through. Required before plugin/trigger/panel can compose.
...
- **2.3 ActionRegistry (Swift native, thin)** — registry shim mapping method → handler. `register_blocking` flag mirrors Phase 9.4. Replace `switch method` in `AppDelegate.handleCommand`. **Pulled forward to PR 2** because plugin host and trigger engine both need a registry seam.
