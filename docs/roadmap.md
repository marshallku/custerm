...
- [x] **Example triggers** at `examples/plugins/jira/triggers.example.toml`: `jira.ticket_assigned → todo.create` (with `linked_jira` populated → composes with vision-flow-3 worktree+claude chain), `jira.mention → kb.ensure` (raw archive at `.raw/jira/<key>-comment-<id>.md` — uses ensure not append-with-ensure for true dedup of Jira-side delivery retries). Plus commented sketches for status_changed → slack.post_message and per-project trigger narrowing via condition.
