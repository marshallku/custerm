...
- [x] Plugin manifest at `examples/plugins/calendar/plugin.toml`. `onStartup` activation (the polling daemon must be alive whenever turm is — `onAction:calendar.*` would only spawn on explicit query, too late for "10 minutes before meeting"). Example trigger config at `examples/plugins/calendar/triggers.example.toml` showing common KB-ensure pattern, per-recurring-event addition (rolling 1:1 note), 60-min focus-block reminder skeleton, and `webview.open` for Meet links — with an explicit "current trigger language limitation" callout at the top so users know that `[triggers.when]` only does positive payload-match: when the same event matches multiple rules, all of them fire, and "skip if X" / attendance-status checks are deferred to Phase 10.2's `condition` clause. 35 unit tests covering env parsing, event JSON mapping (all-day timezone-correct via local-tz interpretation, recurring_id, conference URL extraction, malformed input rejection), token store roundtrip with 0600 perms verification, concurrent-save isolation via per-call atomic counter, account-label charset validation against path traversal, OAuth expiry detection, poller windowing + dedup logic, fired-set cap.

**10.2 — Per-event customization ⏳ (deferred)**

The Phase 10 design conversation surfaced four shapes of per-event customization the user wants:

1. Common actions across events (already handled by existing `[[triggers]]` matching on `event_kind`)
2. Per-recurring-event differentiation (already expressible — `[triggers.when]` payload-match on `recurring_id`)
3. Disable common actions for specific events
4. Conditional execution based on attendance/response status

(3) and (4) collapse into a single primitive: a `condition` clause on a trigger that's an expression evaluated against the event payload + context (`event.my_response_status != "declined" && event.recurring_id != "weekly-1on1"`). Adding it requires a small expression evaluator on `TriggerEngine`. **Deferred** to its own phase because the Phase 10.1 work is already a vertical slice that ships value (calendar events fire matching triggers via the existing engine), and the expression DSL deserves its own design pass: should it be a custom mini-language, or an external crate (`evalexpr`), and how does it interact with `{event.X}` interpolation already in `params`?
