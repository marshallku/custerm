...
**11.2 — Raw archive + write actions ⏳ (deferred)**

- [ ] Raw archive of every Socket Mode payload to `~/docs/.raw/slack/<team>/<channel>/<ts>.json` via the KB plugin's `kb.append` action. Lets the LLM plugin (Phase 12) ingest historical context without re-fetching from Slack. Triggered by user trigger config rather than baked into the slack plugin so KB is not a hard dependency.
- [ ] `slack.post_message` / `slack.add_reaction` / `slack.update_message` action(s) — opens write capability so triggers can compose replies. Requires `chat:write` OAuth scope.
- [ ] Full OAuth redirect flow as an alternative to the env-paste setup — needs a localhost listener for the redirect URI; defer until env+keyring proves insufficient in practice.
